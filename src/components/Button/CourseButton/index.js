...
const CourseButton = ({ item }) => {
  ...
  async function handleDeleteUser(item) {
    ...
    if (confirm(`Delete course ${item.name} ?`) == true) {
      const data = await deleteCourse(item._id);
      if (data?.success) {
    ...
    <div className="flex gap-5">
      ...
      <Button
        ...
        onClick={() => {
          ...
          router.push("/admin-view/course/lesson");
          // console.log("pass item", currentUpdate);
        }}
        // className="text-"
        // className="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:focus:ring-yellow-900"
      >
        <FaEye color="green" className=""></FaEye>
      </Button>
    </div>
  ) : (
    <>null</>
